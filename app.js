...
function addMessage(text, role='assistant', tags=[], speak=false){
  ...
  const bubble=document.createElement('div'); bubble.className='bubble'; bubble.textContent=text; msg.appendChild(bubble);
  if(tags.length){ const row=document.createElement('div'); row.className='tag-row'; row.innerHTML=tags.map(x=>`<span class="tag ${x.toLowerCase()}">${esc(x)}</span>`).join(''); msg.appendChild(row); }
  if(role==='assistant' && 'speechSynthesis' in window){ const actions=document.createElement('div'); actions.className='msg-actions'; const button=document.createElement('button'); button.className='icon-btn'; button.textContent='🔊 Read aloud'; button.onclick=()=>speakText(text); actions.appendChild(button); msg.appendChild(actions); }
...
async function askGemini(question, context){
  const key = 'AQ.Ab8RN6J0oEOYCHnFGkC6CELnTt2FrKQbJeuvGX5GJmgIQ8yMZQ';
  const prompt = `You are WeatherGPT. Give concise, safety-conscious weather guidance in ${lang}. Forecast data: ${context}\n\nUser Question: ${question}`;
...
async function send(question){
  ...
  try { if(!weather){ const query=$('locInput').value.trim() || 'Madurai'; await fetchWeather(await resolveLocation(query)); }
    const brief=localBriefing(question); 
    
    // === NOW CALLS GEMINI ===
    let answer; try { answer=await askGemini(question,brief); } catch { answer=null; }
